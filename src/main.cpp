...
int main(int argc, char *argv[]) {
    ...
    arguments.add_argument("--algorithm").default_value(std::string{"dijkstra"})
        .help("Algorithm to use. Possible variants: dijkstra, astar");

    arguments.add_argument("--debug")
        .help("If set, print debug information [path, used vertexes]")
        .flag();

    arguments.add_argument("--graph_file").required()
        .help("Input graph file");
    
    arguments.add_argument("--coords_file").required()
        .help("Input coordinates file");

    arguments.add_argument("--from").required()
        .help("Algorithm's start point [integer from 0 to graph size]")
        .scan<'i', int>();

    arguments.add_argument("--to").required()
